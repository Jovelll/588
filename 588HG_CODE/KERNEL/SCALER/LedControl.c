...
#if CHIP_ID==CHIP_TSUMC||CHIP_ID==CHIP_TSUMK||CHIP_ID ==CHIP_TSUMD||CHIP_ID==CHIP_TSUMJ   
	msWriteByteMask(REG_0AEF, (CSA_gain==1)?(0x00):(BIT5), BIT5|BIT4);   	//100m△1x,  33m△3x   //[5:4]CSA gain selection 2'10->33m 2'b01->100m
...
#endif
msWrite2ByteMask(REG_0A6E,	509 ,0x1FF); //GDRV voltage wdog,18us x (0~511)程秅戳9ms
    ...
    #endif
#endif
    msWrite2ByteMask(REG_0A66,  511	,0x1FF); //thermal wdog,18us x (0~511)程秅戳9ms
...
void msLED_DimmingPWMAdjust(BYTE PwmValue)
{
    ...
    msWrite2Byte(REG_0AB4, 0x1FF);
    msWrite2Byte(REG_0AB6, 0x1FF);
    msLED_ReloadDimmingPWM();  //ゼ秨Τ快猭 update dimmingpwm盾 //20110303
