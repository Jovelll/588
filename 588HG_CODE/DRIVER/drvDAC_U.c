...
void msAudioDAC_Init( void )
{
    ...
    */
	...
	#if ENABLE_DAC_DEPOP
	msWrite2ByteMask( REG_2C6A, 0, BIT6|BIT4 );
	ucDePOP4SecCounter=250;//delay 2.5 second
	#else
    msWrite2Byte(REG_2C62, 0x2F06);  // 20121003 add for EarPhone out
    msWrite2Byte(REG_2C6A, 0x6406);  // 20121003 add for EarPhone out
	#endif
...
#if ENABLE_DAC_DEPOP
void msAudioDACHandler(void)
{
	if(EARDePopFlag)
	{
		EARDePopFlag = 0; //clear flag
	
		switch (EARDePopStep)
		{
						
			case 1:
				msWrite2ByteMask(REG_2C6A, 0x6406,0xFFFF);
				printMsg("audio depop 1 \r\n");
				EARDePopStep = 2;
				ucDePOP4SecCounter = 100;
				break;
			case 2:
				msWrite2ByteMask(REG_2C62, 0x2C06,0xFFFF);  
				printMsg("audio depop 2 \r\n");
				EARDePopStep = 3;
				ucDePOP4SecCounter = 100;
				break;

			case 3:
				msWrite2ByteMask(REG_2C62, 0x2F06,0xFFFF);
				printMsg("audio depop 3 \r\n");
				EARDePopStep = 0;
				ucDePOP4SecCounter = 0;
				break;
