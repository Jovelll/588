...
void drvGammaLoad320ByteTbl(BYTE channel, BYTE *tblPtr)
{
...
#else
void drvGammaLoadCompressedTbl(BYTE channel, BYTE* ucTab1, BYTE* ucTab2)
{
    WORD u16CurVal = 0;
    WORD u16OutCnt = 0;
    BYTE u8Inc, u8Len;
...
void drvGammaLoadTbl( BYTE GamaMode )
{
    ...
    while( u8TgtChannel < 3 )
    {
            ...
            drvGammaLoad320ByteTbl((u8TgtChannel<<2), pu8Tab1);
#else
            BYTE* pu8Tab1;
            BYTE* pu8Tab2;
            ...
            drvGammaLoadCompressedTbl((u8TgtChannel<<2), pu8Tab1, pu8Tab2 );
#endif
        u8TgtChannel += 1;
    }
}
