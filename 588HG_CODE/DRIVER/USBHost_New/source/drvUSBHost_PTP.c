...
ASSIGNVAR   xAssignVar;
DYMNVAR     xDymnVar;
HOST_PTP_DRIVER PTP_Driver;
//OTGH_PTP_CONTAINER xdata  PTP_Cmd _at_ HOST20_PTP_CONTAINER;
static U16 gPtpBak_xdwin1_Base=0xFFFF; //use 0xFFFFFFFF to represent void

extern U8 RecoveryFlag;
extern U8 gSpeed;
extern PTP_OBJECTINFO     gPtpObjectInfo;

code UINT8 PTP_GET_DEVICE_STATUS[8] = {0xA1,0x67,0x00,0x00,0x00,0x00,0x04,0x00};
code UINT8 USB_GET_ENDPOINT_STATUS[8] = {0x82,0x00,0x00,0x00,0x00,0x00,0x02,0x00};
code UINT8 USB_CLEAR_ENDPOINT_FEATURE[8] = {0x02,0x01,0x00,0x00,0x00,0x00,0x00,0x00};
...
extern U32 drvUSBHost_PTP_GetPtpObjHandlesBufAdr();

//--------------------------------------------------------------------
//  Local Function
//--------------------------------------------------------------------
void drvUSBHost_PTP_Error_Handle();

void drvUSBHost_PTP_SetObjHWindow1(U32 u32PhyAddr)
{
    gPtpBak_xdwin1_Base = (XBYTE[XDMIU_REG_BASE + 13]<<8 | XBYTE[XDMIU_REG_BASE + 12]);

    XBYTE[XDMIU_REG_BASE + 13] = (u32PhyAddr>>12)>>8;
    XBYTE[XDMIU_REG_BASE + 12] = (u32PhyAddr>>12)>>0;
}

void drvUSBHost_PTP_FreeObjHWindow1(void)
{
    XBYTE[XDMIU_REG_BASE + 13] = gPtpBak_xdwin1_Base >> 8;
    XBYTE[XDMIU_REG_BASE + 12] = gPtpBak_xdwin1_Base >> 0;

    gPtpBak_xdwin1_Base=0xFFFF;
}


void drvUSBHost_PTP_Transaction(
    ...
    BOOLEAN bNeedResp)
{
    ...
    if(drvUSBHost_PTP_SendOpCode ( pAssignVar, pDymnVar)!=HOST20_OK)
    {
        ...
        while(1)/*While Looping_Free:<Exit counter added>*/
        {
            ...
            result=drvUSBHost_PTP_SendOpCode ( pAssignVar, pDymnVar);
    ...
    drvUSBHost_PTP_BulkInData ( pAssignVar, pDymnVar);
    if (OTGH_PT_BLK.bStatus != HOST20_OK)
        goto Func_Done;
...
U8 drvUSBHost_PTP_SendOpCode(ASSIGNVAR xdata *pAssignVar,DYMNVAR xdata *pDymnVar)
{
    ...
    u8result = flib_Host20_Issue_Bulk (OTGH_PT_BLK.bOutQHDArrayNum,CPU2Le32(pPTP_Buf->u32ConLen),u32BufAddr,OTGH_Dir_Out);
    DBG_PTP(printf("SendOpCode end,0x%bX\r\n",u8result));
    OTGH_PT_BLK.bStatus = u8result;
    return(u8result);
} //PTP_SendOpCode
...
*
*********************************************************************/
void drvUSBHost_PTP_ChkZeroPacket(U32 u32DataLen)
{
    U32     u32BufAddr;
    U16     u16MaxPacketSize;

    u16MaxPacketSize=sAttachDevice.saCD[0].sInterface[0].sED[0].bED_wMaxPacketSizeHighByte;
    u16MaxPacketSize=(u16MaxPacketSize<<8)+(sAttachDevice.saCD[0].sInterface[0].sED[0].bED_wMaxPacketSizeLowByte);

    if ( (u32DataLen > 0) && (u32DataLen % u16MaxPacketSize == 0) )
    {
        printf("In zero packet\n");
        u32BufAddr = VirtoPhyAddr(sAttachDevice.bDataBuffer);
        OTGH_PT_BLK.bStatus = flib_Host20_Issue_Bulk (OTGH_PT_BLK.bInQHDArrayNum,1,u32BufAddr,OTGH_Dir_IN);
    }
}
...
*
*********************************************************************/
void drvUSBHost_PTP_BulkInData  (ASSIGNVAR xdata *pAssignVar,DYMNVAR xdata *pDymnVar)
{
    U8 xdata *pBuf=(U8 xdata *)(sAttachDevice.bDataBuffer);
    ...
    case PTP_GetStorageIDs:
    case PTP_GetStorageInfo:
        drvUSBHost_PTP_Read1stBulkIn( pAssignVar, pDymnVar );
        if (OTGH_PT_BLK.bStatus!=HOST20_OK)
            goto Func_Done;

        drvUSBHost_PTP_ReadRemBulkIn( pAssignVar, pDymnVar );
        if (OTGH_PT_BLK.bStatus!=HOST20_OK)
            goto Func_Done;

        drvUSBHost_PTP_ChkZeroPacket(pDymnVar->u32DataLen);
        break;
    //case PTP_GetObject:
        //get 12 bytes header
       //  flib_Host20_Issue_Bulk(OTGH_PT_BLK.bInQHDArrayNum, 512,pDymnVar->u32DramAddr,OTGH_Dir_IN);
        //get real data
    ...
    case PTP_GetObject:
    case PTP_GetThumb:
        drvUSBHost_PTP_GetBulkIn( pDymnVar );
        break;
...
*
*********************************************************************/
void drvUSBHost_PTP_Read1stBulkIn(ASSIGNVAR xdata *pAssignVar,DYMNVAR xdata *pDymnVar)
{
    //U8 u8result;
    U8 str_len;
    U8 str_idx;
    ...
    OTGH_PT_BLK.bStatus = flib_Host20_Issue_Bulk(OTGH_PT_BLK.bInQHDArrayNum, u16MaxPacketSize,VirtoPhyAddr(sAttachDevice.bDataBuffer),OTGH_Dir_IN);
    if ( OTGH_PT_BLK.bStatus != HOST20_OK)
    {
    ...
    case PTP_GetStorageInfo:
        ...
        PTP_Driver.u32CapacityL = LE2ME32( *(U32*)(&pBuf[18]));
        PTP_Driver.u32CapacityH = LE2ME32( *(U32*)(&pBuf[22]));
        PTP_Driver.u32FreeSpaceL= LE2ME32( *(U32*)(&pBuf[26]));
        PTP_Driver.u32FreeSpaceH= LE2ME32( *(U32*)(&pBuf[30]));
        //printf("u32CapacityH:0x%lX\r\n",PTP_Driver.u32CapacityH);
        //printf("u32CapacityL:0x%lX\r\n",PTP_Driver.u32CapacityL);
        break;
    case PTP_GetObjectInfo:
        //-----------------------------------------------------------------
    ...
    case PTP_GetObjectHandles:
        {
            ...
            PTP_Driver.u32TotalHandleNo = ReadU32LE(&pBuf[12]);

            //PTP_Driver.u32CurHandleIdx = (PTP_Driver.u32FirstHandleNo);
            DBG_PTP(printf("Num=%lx ,Id=%lx \n",PTP_Driver.u32TotalHandleNo,PTP_Driver.u32FirstHandleNo));
            PTP_Driver.u32CurHandleIdx=0;
            for(u16Idx=16; u16Idx<u16MaxPacketSize; u16Idx+=4)
            {
                u32Tmp = ReadU32LE(&pBuf[u16Idx]);
                drvUSBHost_PTP_SaveObjectHandleNo(PTP_Driver.u32CurHandleIdx, u32Tmp);
                PTP_Driver.u32CurHandleIdx++;
            }
        break;
        }
...
*
*********************************************************************************/
void drvUSBHost_PTP_ReadRemBulkIn(ASSIGNVAR xdata *pAssignVar,DYMNVAR xdata *pDymnVar )
{
    U8 loop_num;
    U8 str_len;
    U8 str_idx; //,u8Result;
...
void drvUSBHost_PTP_GetBulkIn(DYMNVAR xdata *pDymnVar)
{
    //U8 u8result;
    U16 TransferLengh;
    ...
    while(wTotalLengthRemain>0)
    {
        ...
        OTGH_PT_BLK.bStatus = flib_Host20_Issue_Bulk(OTGH_PT_BLK.bInQHDArrayNum, TransferLengh,
        ...
        Offset+=TransferLengh-TotalBytes;       // TotalBytes is remaining bytes return from flib_Host20_Issue_Bulk
        wTotalLengthRemain-=TransferLengh-TotalBytes;
        //   printf("oNE TIME TransferLengh:%x\n",TransferLengh-TotalBytes);
        gTotalBytes+=TransferLengh-TotalBytes;
        if (TotalBytes) break;          //short packet end
    }
...
*********************************************************************/
U8 drvUSBHost_PTP_BulkInResponse(void)
{
    U8 rtn_val,u8result;
    U32 u32BufAddr;
    ...
    u32BufAddr = VirtoPhyAddr(sAttachDevice.bDataBuffer);
   // printf("resbuf:%lx\n",u32BufAddr);
    u8result = flib_Host20_Issue_Bulk (OTGH_PT_BLK.bInQHDArrayNum,12,u32BufAddr,OTGH_Dir_IN);
    ...
    for (i=0; i<12; i++ )
    {
        DBG_PTP(printf("%bx, ",((U8 xdata*)sAttachDevice.bDataBuffer)[i]));
    }
    DBG_PTP(printf(("\r\n")));
    */
    if( (((U8 xdata*)sAttachDevice.bDataBuffer)[0] != 0x0c) || (((U8 xdata*)sAttachDevice.bDataBuffer)[6] != 0x01) || ( ((U8 xdata*)sAttachDevice.bDataBuffer)[7] != 0x20 ) )
    {
...
U8 drvUSBHost_PTP_Initial(void)
{
    DBG_PTP(printf("drvUSBHost_PTP_Initial\r\n"));    
    OTGH_PT_Bulk_Init();
    
    memset( (void*)(&PTP_Driver), 0, sizeof(PTP_Driver) );

    PTP_Driver.u32ObjectHandleNumBuf = drvUSBHost_PTP_GetPtpObjHandlesBufAdr();
    DBG_PTP(printf("Handle Buf Addr:%lx\n", PTP_Driver.u32ObjectHandleNumBuf ));
    ...
    xDymnVar.u16FileType  = 0;
    xDymnVar.u32DataLen   = 0;
    xDymnVar.u32StorageId = 0;
    //xDymnVar.u32HandleNum = 0;
    xDymnVar.u32HandleId  = 0;
    MDrv_Timer_Delayms(100);    // Delay for DSC ready
    xAssignVar.u16OpCode = PTP_OpenSession;
    xAssignVar.u32TransId = 0x00;
    drvUSBHost_PTP_Transaction(&xAssignVar, &xDymnVar, TRUE);

    xAssignVar.u16OpCode = PTP_GetDeviceInfo;
    xAssignVar.u32TransId++;
    drvUSBHost_PTP_Transaction(&xAssignVar, &xDymnVar, TRUE);

    xAssignVar.u16OpCode = PTP_GetStorageIDs;
    xAssignVar.u32TransId++;
    drvUSBHost_PTP_Transaction(&xAssignVar, &xDymnVar, TRUE);

    xAssignVar.u16OpCode = PTP_GetStorageInfo;
    xAssignVar.u32TransId++;
    drvUSBHost_PTP_Transaction(&xAssignVar, &xDymnVar, TRUE);
    ...
    //-------------------------------------
    xAssignVar.u16OpCode = PTP_GetObjectHandles;
    xAssignVar.u32TransId++;
    xAssignVar.u32Param1 = 0xFFFFFFFF;
    xAssignVar.u32Param2 = 0;
    xAssignVar.u32Param3 = 0;
    drvUSBHost_PTP_Transaction (&xAssignVar, &xDymnVar);
    DBG_PTP(printf("\r\n\r\nHandleNo=0x%lX\r\n",PTP_Driver.u32TotalHandleNo));
    printf("\r\n\r\nHandleNo=0x%lX\r\n",PTP_Driver.u32TotalHandleNo);
#endif
#if 0
    {
        ...
        ObjCount = drvUsbPTP_GetObjectHandles(USB_PORT_0, 0, 0);

        printf("ObjCount: %ld\n", ObjCount);
        for (ii=0; ii<ObjCount; ii++)
        {
            ObjHandle = drvUsbPTP_GetObjectHandleByIndex(USB_PORT_0, ii);
            pObjInfo = drvUsbPTP_GetObjectInfo(USB_PORT_0, ObjHandle);
            ...
            {
                pBuf4GetTest = PTP_Driver.u32ObjectHandleNumBuf + 32768;
                ...
                while (GetOffset < pObjInfo->ThumbCompressedSize)
                {
                    ...
                    reallen = drvUsbPTP_GetThumb(USB_PORT_0, ObjHandle, GetOffset, transLen, pBuf4GetTest);
                    printf("reallen: %ld\n", reallen);
                ...
                while (GetOffset < pObjInfo->ObjectCompressedSize)
                {
                    ...
                    printf("GetOffset: %ld, transLen: %ld\n", GetOffset, transLen);
                    reallen = drvUsbPTP_GetObject(USB_PORT_0, ObjHandle, GetOffset, transLen, pBuf4GetTest);
                    printf("reallen: %ld\n", reallen);
#if 0
    ...
    idx=0;
    idy=0;
     while(idx < (PTP_Driver.u32TotalHandleNo))
    {
     ...
     PTP_Driver.u32TotalHandleNo=idy;
     printf("after fitering size:%lx\n",idy);
 #endif
    #if 0
    for( i=0 ; i < xDymnVar.u32HandleNum  ; i++ )
    {
        DBG_PTP(printf("i=%lx, Id=%lx\n",i,xDymnVar.u32HandleId ));
        //--------------------------------
        // Get the File Info
        //--------------------------------
        xAssignVar.u16OpCode = PTP_GetObjectInfo;
        xAssignVar.u32TransId++;
        xDymnVar.u16FileType = UnKnownFile;
        drvUSBHost_PTP_Transaction (&xAssignVar, &xDymnVar);
        DBG_PTP(printf("FileNand:"));
        for(j=0;j<8;j++)
        {
            if(xDymnVar.u8FileName[j] == NULL)
                DBG_PTP(printf(" "));
            else
                DBG_PTP(printf("%C",xDymnVar.u8FileName[j]));
        }
        DBG_PTP(printf("."));
        for(j=0;j<3;j++)
        {
            if(xDymnVar.u8ExtName[j] == NULL)
                DBG_PTP(printf(" "));
            else
                DBG_PTP(printf("%C",xDymnVar.u8ExtName[j]));
        }
        DBG_PTP(printf("\r\n"));
        //--------------------------------
        // Check the file type
        //--------------------------------
        if( xDymnVar.u16FileType == JpegFile )
        {
            MDrv_GetXdataWindow1Map(&u8Win1StarAddr, &u8Win1EndAddr, &u32Win1PhyAdr);
            MDrv_MapXdataWindow1(60,64,(((BASEADR_SDRAM_USB_DEV_BUF)+0xFFF) & ~0xFFF)+PTP_Driver.u16ValidHandleNo*32);
            memset(pDummyDirEntry,0x00,0x20);
            PTP_Driver.u16ValidHandleNo++;
            memcpy(pDummyDirEntry->ps8DirEntryName,xDymnVar.u8FileName,8);
            memcpy(pDummyDirEntry->ps8DirEntryExtName,xDymnVar.u8ExtName,3);
            pDummyDirEntry->u32DirEntryFileLength = CPU2Le32(xDymnVar.u32FileLength);
        ...
        //{
        //  xAssignVar.u16OpCode  = GetObject;
        //  xAssignVar.u32TransId++;
        //  PTP_Transaction (&xAssignVar, &xDymnVar );
        //}
        xDymnVar.u32HandleId++;
    }
    #endif
    //xAssignVar.u16OpCode = PTP_CloseSession;
    //xAssignVar.u32TransId ++;
    //drvUSBHost_PTP_Transaction(&xAssignVar, &xDymnVar);
    return(PTP_OK);
}
...
{
  U32 u32Tmp = drvUSBHost_PTP_GetObjectHandleNo(PTP_Driver.u32CurHandleIdx);
...
DYMNVAR *drvUSBHost_PTP_GetObjectInfo(U32 u32HandleNo )
{
    U8     u8Status;

    DBG_PTP(printf("drvUSBHost_PTP_GetObjectInfo\r\n"));
    //printf("u32HandleNo:%lx\n",u32HandleNo);
    //printf("u32FirstHandleNo:%lx\n",PTP_Driver.u32FirstHandleNo);

    xAssignVar.u16OpCode = PTP_GetObjectInfo;
    xAssignVar.u32TransId++;
    xDymnVar.u16FileType = 0;
    xDymnVar.u32HandleId = u32HandleNo;
...
DYMNVAR *drvUSBHost_PTP_GetFirstFileObjectInfo(void)
{
    ...
    while(PTP_Driver.u32CurHandleIdx < (PTP_Driver.u32TotalHandleNo-1))
    {
        DBG_PTP(printf("u32CurHandleIdx=0x%lX\r\n",PTP_Driver.u32CurHandleIdx));
        u32Tmp = drvUSBHost_PTP_GetObjectHandleNo(PTP_Driver.u32CurHandleIdx);
      //  printf("Handle is :%lx\n",u32Tmp);
        if(u32Tmp==INVALID_HANDLE_NO)
            return(NULL);
        pdymnvar = drvUSBHost_PTP_GetObjectInfo(u32Tmp);
        ...
        if(pdymnvar->u16FileType != OBJ_ASSOCIATION)
        {
            PTP_Driver.u32FirstFileHandleNo = drvUSBHost_PTP_GetObjectHandleNo(PTP_Driver.u32CurHandleIdx);
            DBG_PTP(printf("u32CurHandleIdx=0x%lX\r\n",PTP_Driver.u32CurHandleIdx));
    //        sprintf(pdymnvar->u8FileName,"PTP");
            u32Tmp = PTP_Driver.u32FirstFileHandleNo;
...
DYMNVAR *drvUSBHost_PTP_GetOffsetFileObjectInfo(U32 offset)
{
        ...
        if ((PTP_Driver.u32CurHandleIdx+offset) > (PTP_Driver.u32TotalHandleNo-1)) return NULL;
        PTP_Driver.u32CurHandleIdx+=offset;
       // printf("curidx:%lx\n",PTP_Driver.u32CurHandleIdx);
        u32Tmp = drvUSBHost_PTP_GetObjectHandleNo(PTP_Driver.u32CurHandleIdx);
        if(u32Tmp==INVALID_HANDLE_NO)
            return(NULL);
...
DYMNVAR *drvUSBHost_PTP_GetNextFileObjectInfo(void)
{
    ...
    while(PTP_Driver.u32CurHandleIdx < (PTP_Driver.u32TotalHandleNo-1))
    {
        ...
        {
           // sprintf(pdymnvar->u8FileName,"PTP");
           // u32Tmp = PTP_Driver.u32CurHandleIdx;
           // pdymnvar->u8FileName[7] = (u32Tmp%10) + '0';
           // u32Tmp /= 10;
            //pdymnvar->u8FileName[6] = (u32Tmp%10) + '0';
          //  u32Tmp /= 10;
...
DYMNVAR *drvUSBHost_PTP_GetPrevFileObjectInfo(void)
{
    ...
    printf("<<GetNxtF-OBJ\r\n");
    while(PTP_Driver.u32CurHandleIdx)
    {
        PTP_Driver.u32CurHandleIdx--;
        u32Tmp = drvUSBHost_PTP_GetObjectHandleNo(PTP_Driver.u32CurHandleIdx);
        if(u32Tmp==INVALID_HANDLE_NO)
...
extern U32  gTotalBytes;
U8   StartGetObject=0;

U32 drvUSBHost_PTP_GetObjectData(
    U16 u16OpCode,
    U32 u32TotalSize,
    U32 u32HandleIdx,
    U32 u32FileOffset,
    ...
    U32 u32DRAMAddr)
{
    ...
    if (u32FileOffset==0)
    {
        ...
        xAssignVar.u16OpCode = u16OpCode;
        xAssignVar.u32TransId++;
        xDymnVar.u32FileOffset = 0;
        xDymnVar.u32DataLen = uTransLen;
        xDymnVar.u32HandleId = u32HandleIdx;
        xDymnVar.u32DramAddr = VirtoPhyAddr(sAttachDevice.bDataBuffer);
        drvUSBHost_PTP_Transaction(&xAssignVar, &xDymnVar, FALSE);
        if (OTGH_PT_BLK.bStatus!=HOST20_OK)
            goto Func_Done;

        MDrv_MIU_Copy(VirtoPhyAddr(sAttachDevice.bDataBuffer) + PTP_GEN_CONTAIN_SIZE,
                u32DRAMAddr, gTotalBytes - PTP_GEN_CONTAIN_SIZE, MIU_SDRAM2SDRAM);

        g32PTPFilePtr += gTotalBytes - PTP_GEN_CONTAIN_SIZE;
        ...
        if ( (u32Length - g32PTPFilePtr) > wMaxPacketSize )
        {
            xAssignVar.u16OpCode = u16OpCode;
            xAssignVar.u32TransId++;
            xDymnVar.u32DataLen = (u32Length - g32PTPFilePtr) & ~((U32)wMaxPacketSize-1);
            xDymnVar.u32HandleId = u32HandleIdx;
            xDymnVar.u32DramAddr = u32DRAMAddr + g32PTPFilePtr;
            drvUSBHost_PTP_BulkInData ( &xAssignVar, &xDymnVar);
            if (OTGH_PT_BLK.bStatus!=HOST20_OK)
                goto Func_Done;

            g32PTPFilePtr += gTotalBytes;
        ...
        if ( (u32Length - g32PTPFilePtr) > 0 )
        {
            ...
            {
                MDrv_MIU_Copy(VirtoPhyAddr(sAttachDevice.bDataBuffer), u32DRAMAddr + g32PTPFilePtr,
            ...
            else
            {
                ...
                g32PTPFilePtr += (gTotalBytes > PTP_GEN_CONTAIN_SIZE) ? PTP_GEN_CONTAIN_SIZE : gTotalBytes;
            }
    ...
    else if (u32FileOffset == g32PTPFilePtr)
    {
        ...
        if (u32Length < (gTotalBytes -PTP_GEN_CONTAIN_SIZE) )
            uTransLen = u32Length;
        else
            uTransLen = gTotalBytes -PTP_GEN_CONTAIN_SIZE;

        MDrv_MIU_Copy(VirtoPhyAddr(sAttachDevice.bDataBuffer) + PTP_GEN_CONTAIN_SIZE, u32DRAMAddr,
                uTransLen, MIU_SDRAM2SDRAM);
        ...
        if ( ((u32Length - uBufOffset) > 0) && (g32PTPFilePtr < u32TotalSize) )
        {
            if ( (u32Length - uBufOffset) > wMaxPacketSize)
            {
                ...
                xDymnVar.u32DataLen = uTransLen;
                xDymnVar.u32HandleId = u32HandleIdx;
                xDymnVar.u32DramAddr = u32DRAMAddr + uBufOffset;
                drvUSBHost_PTP_BulkInData ( &xAssignVar, &xDymnVar);
                if (OTGH_PT_BLK.bStatus!=HOST20_OK)
                    goto Func_Done;

                g32PTPFilePtr += gTotalBytes;
                uBufOffset += gTotalBytes;
            ...
            if ( ((u32Length - uBufOffset) > 0) && (g32PTPFilePtr < u32TotalSize) )
            {
                ...
                {
                    MDrv_MIU_Copy(VirtoPhyAddr(sAttachDevice.bDataBuffer), u32DRAMAddr + uBufOffset,
                            uTransLen, MIU_SDRAM2SDRAM);
                    g32PTPFilePtr += uTransLen;
                    uBufOffset += uTransLen;
...
Get_Done:
    ...
    if ( g32PTPFilePtr >=  (u32TotalSize) )
    {
        ...
        drvUSBHost_PTP_ChkZeroPacket((U32)g32PTPFilePtr+PTP_GEN_CONTAIN_SIZE);
        if (OTGH_PT_BLK.bStatus != HOST20_OK)
            goto Func_Done;
    ...
    U32 u32DRAMAddr)
{
    ...
    if (u32FileOffset==0)
    {
        StartGetObject=1;
        g32PTPFilePtr=0;
        xAssignVar.u16OpCode = PTP_GetObject;
        xAssignVar.u32TransId++;
        xDymnVar.u32FileOffset = u32FileOffset;
        xDymnVar.u32DataLen = u32Length;
        xDymnVar.u32HandleId = u32HandleIdx;
        xDymnVar.u32DramAddr = u32DRAMAddr;

        drvUSBHost_PTP_Transaction2 (&xAssignVar, &xDymnVar);
        g32PTPFilePtr+=gTotalBytes;
        //printf(" Trans:%lx",gTotalBytes);
    }
    else if (u32FileOffset==g32PTPFilePtr)
    {
...
void GetResidualBulkIn(U32 u32Total)
{
   ...
   if (StartGetObject==0) return;

    while (1)
        {
            u8result = flib_Host20_Issue_Bulk(OTGH_PT_BLK.bInQHDArrayNum, 16*1024,VirtoPhyAddr(BASEADR_SDRAM_USB_DUMMY_BUF),OTGH_Dir_IN);

            g32PTPFilePtr+=0x4000-TotalBytes;
       //     printf("cleanup-> fileptr:%lx , totalsize:%lx \n",g32PTPFilePtr,u32Total);

            if (TotalBytes) break;      //break when short packet
            if (g32PTPFilePtr >= u32Total ) break;     //BEAK WHEN data is engough

        }
    flib_Host20_Issue_Bulk(OTGH_PT_BLK.bInQHDArrayNum, 12,VirtoPhyAddr(sAttachDevice.bDataBuffer),OTGH_Dir_IN);
    StartGetObject=0;
}
...
{
    U32 u32Tmp;
    u32Tmp = (PTP_Driver.u32FreeSpaceH<<23)+(PTP_Driver.u32FreeSpaceL/200);
    return(u32Tmp);
}
#endif

U32 drvUSBHost_PTP_GetObjectHandleNo(U32 u32HandleIdx)
{
    U32 u32HandleNo;

    #if (PLATFORM==MCU8051)
    U32 u32Win1Adr = PTP_Driver.u32ObjectHandleNumBuf;
    U32 xdata *pu32HandleBuf = (U32 xdata*)0xF000;
    #endif

    if(u32HandleIdx>=PTP_Driver.u32TotalHandleNo)
    ...
    #if (PLATFORM==MCU8051)
    drvUSBHost_PTP_SetObjHWindow1(u32Win1Adr);
    #endif
...
void drvUSBHost_PTP_SaveObjectHandleNo(U32 u32HandleIdx,U32 u32HandleNo)
{
    #if (PLATFORM==MCU8051)
    U32 u32Win1Adr = PTP_Driver.u32ObjectHandleNumBuf;
    U32 xdata *pu32HandleBuf = (U32 xdata*)0xF000;
...
U32 drvUSBHost_PTP_GetObjectHandles(U16 u16ObjFormat, U32 u32AssociObjHandle)
{
    ...
    xAssignVar.u32TransId++;
    xAssignVar.u32Param1 = 0xFFFFFFFF;
    xAssignVar.u32Param2 = u16ObjFormat;
    xAssignVar.u32Param3 = u32AssociObjHandle;
    ...
    return (PTP_Driver.u32TotalHandleNo);
}

BOOLEAN drvUSBHost_PTP_GetObjInfo(U32 u32ObjectHandle)
{
    U8          ii;
    DYMNVAR     *pDymnVar;

    pDymnVar = drvUSBHost_PTP_GetObjectInfo(u32ObjectHandle);
...
U32 drvUSBHost_PTP_GetCurGetObjOffset(void)
{
    return g32PTPFilePtr;
}

U8 drvUSBHost_PTP_GetError(void)
{
    U8 u8UsbStatus;
    ...
    u8UsbStatus = OTGH_PT_BLK.bStatus;
...
void drvUSBHost_PTP_Error_Handle()
{
    ...
    memcpy(bCMD, PTP_GET_DEVICE_STATUS,8);
    ...
    memcpy(bCMD, USB_GET_ENDPOINT_STATUS, 8);
    bCMD[4] = sAttachDevice.saCD[0].sInterface[0].sED[0].bED_EndpointAddress & 0x0f;
    if ( (flib_Host20_Issue_Control (1, bCMD, 2,  u8Buff) == 0) && (u8Buff[0] != 0) )
    {
        printf("Get Ept %bd Status: 0x%bX\n", bCMD[4], u8Buff[0]);
        memcpy(bCMD, USB_CLEAR_ENDPOINT_FEATURE, 8);
        bCMD[4] = sAttachDevice.saCD[0].sInterface[0].sED[0].bED_EndpointAddress & 0x0f;
        flib_Host20_Issue_Control (1, bCMD, 0,  NULL);
    }

    memcpy(bCMD, USB_GET_ENDPOINT_STATUS, 8);
    bCMD[4] = sAttachDevice.saCD[0].sInterface[0].sED[1].bED_EndpointAddress & 0x0f;
    if ( (flib_Host20_Issue_Control (1, bCMD, 2,  u8Buff) == 0) && (u8Buff[0] != 0) )
    {
