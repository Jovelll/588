...
#define INT_FIQ_WDT_ENABLE(Enable)    (msWriteByteMask( REG_2B00, ((Enable)?(0):(BIT2)), BIT2 ))
...
#define INT_IRQ_LEDSARINT_ENABLE(Enable)    (msWriteByteMask( REG_2B18, ((Enable)?(0):(BIT5)), BIT5 ))
#define INT_STATUS_LEDSARINT_ISR()               (_bit5_(msRegs[REG_2B28])) 
...
#define HDCP_CLEAR_STATUS()     (msWriteByte( REG_28C3, 0xFF ))
...
#define HDCP_RESET_ISR(Enable)      (MEM_MSWRITE_BYTE_MASK(REG_290E , (Enable)?(BIT5):(0), BIT5))

// dvi
#define DVI_A_RCK_CTRL(On)  (msWriteByteMask( REG_PM_B2, ((On) ? 0 : BIT0), BIT0 ))
#define DVI_B_RCK_CTRL(On)  (msWriteByteMask( REG_PM_B2, ((On) ? 0 : BIT1), BIT1 ))// check 
#define DVI_C_RCK_CTRL(On)  // none
...
#define HISTOGRAM_RGB_TO_Y_EN(Enable)   (msWriteByteMask(SC3_3B, (Enable)?(BIT6):(0), BIT6)) // check   
#define MWE_FUNCTION_EN(Enable)   (msWriteByteMask(SC0_5C, (Enable)?(BIT3):(0), BIT3))
#define STATISTIC_REQUEST_MAIN_ENABLE()  (msWriteByte(SC3_B8, (BIT2|BIT1)))
#define STATISTIC_REQUEST_SUB_ENABLE()  (msWriteByte(SC3_B8, (BIT2|BIT0)))
#define STATISTIC_REQUEST_EN(Enable) (msWriteByteMask(SC3_B8, (Enable)?(BIT2):(0), BIT2))
#define STATISTIC_ACKNOWLEDGE() ((msReadByte(SC3_B8)&BIT3) == BIT3)
#define Y_CHANNEL_MIN_PIXEL()   (msReadByte( SC3_39 ))
#define Y_CHANNEL_MAX_PIXEL()   (msReadByte( SC3_3A ))
#define TOTAL_PIXEL_COUNT()     (msRead2Byte(SC3_34))
#define TOTAL_PIXEL_WEIGHT()    (msRead2Byte(SC3_36))
...
#define ICE_MAIN_BRI_CTRL(Enable)   (msWriteByteMask(SC6_30, (Enable)?(BIT6):(0), BIT6))
#define ICE_MAIN_SAT_CTRL(Enable)   (msWriteByteMask(SC6_30, (Enable)?(BIT4):(0), BIT4))
#define ICE_MAIN_HUE_CTRL(Enable)   (msWriteByteMask(SC6_30, (Enable)?(BIT5):(0), BIT5))
#define ICE_SUB_BRI_CTRL(Enable)   (msWriteByteMask(SC6_30, (Enable)?(BIT2):(0), BIT2))
#define ICE_SUB_SAT_CTRL(Enable)   (msWriteByteMask(SC6_30, (Enable)?(BIT0):(0), BIT0))
#define ICE_SUB_HUE_CTRL(Enable)   (msWriteByteMask(SC6_30, (Enable)?(BIT1):(0), BIT1))
#define ICE_MAIN_CTRL(Enable)   (msWriteByte(SC6_30, (Enable)?(BIT6|BIT5|BIT4):(0), (BIT6|BIT5|BIT4)))     
#define ICE_SUB_CTRL(Enable)   (msWriteByte(SC6_30, (Enable)?(BIT2|BIT1|BIT0):(0), (BIT2|BIT1|BIT0)))    
#define ICE_DEFINE_RANGE_RGB(Color, Value)  (msWriteByte(SC6_31+(Color), Value))   
#define ICE_ACTIVE_RANGE_RGBCMY(Color, Value)   (msWriteByte(SC6_34+(Color), Value))
#define ICE_SATURATION_RGBCMY(Color, Value) (msWriteByte(SC6_3A+(Color), Value))
#define ICE_HUE_RGBCMY(Color, Value)    (msWriteByte(SC6_40+(Color), Value))
#define ICE_BRIGHTNESS_RGBCMY(Color, Value)    (msWriteByte(SC6_46+(Color), Value))

#define CSC_MAIN_ENABLE(Enable) (msWriteByteMask(SC0_02, (Enable)?(BIT3):(0), BIT3))   
#define CSC_SUB_ENABLE(Enable) (msWriteByteMask(SC0_5A, (Enable)?(BIT6):(0), BIT6))   
...
#define OSD_WRITE_FONT_ADDRESS(u8Addr)  msWrite2Byte(OSD1_66, GET_FONT_RAM_ADDR(u8Addr));
