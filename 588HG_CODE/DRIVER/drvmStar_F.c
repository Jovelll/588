...
#define ENABLE_10BIT_GAMMA_OUTPUT	1
#if ENABLE_10BIT_GAMMA_OUTPUT
void drvmStar_10bitGamma_Output(void);
#endif
...
code RegUnitType tblInit[]=
{
    ...
    {SC0_49, 0xAF},
    {SC5_3E, 0x40},
    {SC5_3F, 0x0A}, // Line buffer size set to 8.2 lines (7.2->0x900, 9.2->0xB80)
#endif
...
void msTMDSInit(void)
{
    ...
    msWriteByteMask(REG_1701, 0x80, 0x80); //De-interskew new mode
#endif
 ...
 #if ENABLE_HDMI
    msWriteByteMask(REG_1F3D, 0x80, 0x80); // Enable auto band select
    msWriteByteMask(REG_1FA3, 0x00, 0x40); // Auto EQ result sel from hdmi or dp
    msWriteByteMask(REG_1BDC, 0x00, 0xC0); // overwrite autoEQ enable
    msWriteByteMask(REG_1BDD, 0x00, 0x03); // overwrite autoEQ enable
    msWriteByteMask(REG_1B2B, 0x00, 0x80); // bypass_symbol_lock_det_coarse
    msWriteByteMask(REG_1B4A, 0x80, 0x80); // Phase lock Timer mode
    msWriteByteMask(REG_1B4B, 0x80, 0x80); // autoEQ enable
    msWriteByteMask(REG_1F8A, 0x11, 0x1F); // coarse tune min value
    msWriteByteMask(REG_1F8B, 0x1F, 0x1F); // coarse tune max value
    msWriteByteMask(REG_1B67, 0x12, 0x3F); // coarse tune start value
    msWrite2Byte(REG_1F84, 0x00FF); // coarse tune symbol det times
    msWrite2Byte(REG_1F4E, 0x0050); // coarse tune symbol aaba number
    msWriteByteMask(REG_1F88, 0x01, 0x01); // enable coarse symbol lock
    msWrite2Byte(REG_1F50, 0x3FFF); // fine tune aaba number
    msWrite2Byte(REG_1F52, 0x0001); // fine tune under threshold
    msWriteByteMask(REG_1F54, 0x01, 0x05); //[2]: reg_under_over_mode, [0]:enable fine tune symbol lock mode
    msWriteByteMask(REG_1B51, 0x00, 0x04); //disable timer mode in fine tune
    msWriteByteMask(REG_1B66, 0x08, 0x08); //enable tmds trigger
    msWrite2Byte(REG_1B52, 0x8000); //coarse tune each step time
    msWriteByteMask(REG_1B4D, 0x0F, 0x3F); //fine tune settel times
    msWriteByteMask(REG_1F54, 0xF0, 0xF0);
    msWrite2Byte(REG_1F56, 0x0350); // find tune continuous symbol number
    msWriteByteMask(REG_1F54, 0x02, 0x02); // find tune continuous symbol enable
    msWriteByteMask(REG_1B4E, 0x03, 0x3F); // reg_phase_stable_threshold
    msWriteByteMask(REG_1B4F, 0x05, 0x3F); // reg_eq_strength_stable_threshold
...
void TMDSISR(void)
{
    ...
    if(_bit5_(msRegs[REG_1757])) // CLK back to stable IRQ
    {
        ...
        temp2 = ((MEM_MSREAD_BYTE(REG_1B78)) | (MEM_MSREAD_BYTE(REG_1B79)<<8));
#if ENABLE_XTAL_LESS
        if(temp2 > 0x5D5)
        {
            MEM_MSWRITE_BYTE_MASK(REG_1F3D, 0x00, 0x80); // Disable Auto Band select EQ
            MEM_MSWRITE_BYTE_MASK(REG_1BB5, 0x01, 0x07); //autoEQ start flag
        }
        else
        {
            MEM_MSWRITE_BYTE_MASK(REG_1F3D, 0x80, 0x80); // Auto Band select EQ
            MEM_MSWRITE_BYTE_MASK(REG_1BB5, 0x00, 0x07); //autoEQ stop flag
        }
#else
        if(temp2 > 0x4E3)
        {
...
static void msSetupDelayLine(void)
{
    ...
    WORD xdata u16Ldly_mid = (8200-(DWORD)u16FV*Ho_Act/Ho_Total-((DWORD)Hi_Blanking*1000)/Ho_Total)/2;
...
void drvmStar_10bitGamma_Output(void)
{
	msWriteByteMask(SC0_32,0x40,0x40);
	msWriteByteMask(SC0_38,0x80,0x80);

	msWriteByte(SC9_D0,0x00);
	msWriteByte(SC9_D1,0x84);

	msWriteByte(SC24_7E,0x03);
	msWriteByte(SC24_8C,0xAA);
	msWriteByte(SC24_8D,0xAA);
	msWriteByte(SC24_8E,0xAA);
	msWriteByte(SC24_8F,0xAA);
	msWriteByte(SC24_90,0xAA);
	msWriteByte(SC24_91,0xAA);
	msWriteByte(SC24_92,0xAA);
	msWriteByte(SC24_93,0xAA);
	msWriteByte(SC24_94,0xAA);
	msWriteByte(SC24_95,0xAA);
