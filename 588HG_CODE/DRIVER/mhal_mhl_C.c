...
code msLoadTbl_S tMHL_HDMI_BYPASS_TABLE[] = 
{
    ...
    {REG_17BC, 0x0040 |MHL_PLL_LDO_VALUE, MASKBIT(15:0)}, // REG_COMBO_ATOP_5E[15:0], ATOP_TEST_PLL, add PLL LDO 10% setting
    ...
    {REG_1504, 0, BIT7}, // REG_COMBO_HDCP_02[7]: PP mode HDCP
...
code msLoadTbl_S tMHL_24BIT_MODE_TABLE[] = 
{
    ...
    {REG_17BC, BIT6 |BIT3 |BIT2 |MHL_PLL_LDO_VALUE, MASKBIT(15:0)}, // REG_COMBO_ATOP_5E[15:0], ATOP_TEST_PLL[3:2] = 2'b 11, add PLL LDO 10% setting
...
{
    {REG_1504, BIT7, BIT7}, // REG_COMBO_HDCP_02[7]: PP mode HDCP
...
Bool _mhal_mhl_CDRCheckPhase(Bool bFlag)
{
    ...
    else if(ucMHLDriver <= MHL_DRIVER_U03)
    {
        if(bFlag)
        {
            if((!GET_MHL_POLLING_TIMER()) && (ucPhase < MHL_PHASE_CHANGE_MASK +1))
            {
                if(ucPhase > 0)
                {
                    if(msReadByte(REG_14EC) & BIT5) // check DE difference
                    {
                        if(ucState == MHL_PHASE_CHECK_START)
                        {
                            msWriteBit(REG_1750, TRUE, BIT0); // TMDS0 clock invert

                            ucState = MHL_PHASE_ERROR_HAPPEN;
                        }
                        else
                        {
                            msWriteBit(REG_1750, FALSE, BIT0); // TMDS0 clock invert
                            
                            ucState = MHL_PHASE_CHECK_START;
                        }
                        
                        MHL_HAL_DPRINTF("** MHL Clock Invert: %d\n", ucState);
                    }
                    else
                    {
                        ucPhase = MHL_PHASE_CHANGE_MASK +1;
                        
                        MHL_HAL_DPUTSTR("** MHL Clock Patch done:\n");
                    }
                }

                ucPhase++;

                msWriteBit(REG_14EC, 1, BIT5); // clear DE difference status

                SET_MHL_POLLING_TIMER(MHL_PHASE_CHECK_INTERVAL);
            }
        }
        else
        {
