#ifndef _DRV_MSHDMI_7_C_
#define _DRV_MSHDMI_7_C_
#include "types.h"
#include "board.h"
#if ENABLE_HDMI
#include "Global.h"
...
void drv_mstar_HDMIInitialAudio( void )
{
    ...
    msWriteByte( REG_1404, 0x03 ); // [1:0]: deep color mode enable
    //msWriteByteMask(REG_0510, 0, BIT4|BIT3); // [4]:0: select pixel clock, [3]:0: manuel pixel clock mode
    msWriteByteMask( REG_142A, BIT6, BIT7 | BIT6 ); // [3]:1: auto pixel clock mode - Montior series

    msWriteByteMask( REG_1707, 0, BIT0 ); // [0]: HPLL power on
    msWriteByteMask( REG_1426, 0xF0, 0xF0 ); // [5]: Enable Port0~Port3 video clock 2x for HDMI deep color 12 bits

#if ENABLE_AUDIO_AUTO_MUTE // auto mute setting
    msWriteByte( REG_1429, 0xAF ); // Fading events: [7]: audio sample error, [5]:Channel Status/present bit error, [3]:no input clock, [2]: HDMI AVMUTE, [1]:clcok big change, [0]: CTS/N over range.
    ...
    msWriteByte( REG_14D4, 0x08 ); // [3:2]: Automatic fading mode
    msWriteByte( REG_14D3, 0x0C ); // [3:2]:fading speed select 8X
    msWriteByte( REG_14D0, 0xFF ); // Zero crossing threshold = 0xFF
    msWriteByte( REG_14D1, 0xFF ); // Zero crossing threshold = 0xFF
    msWriteByte( REG_14D2, 0xAF ); // Fading events: [7]: audio sample error, [5]:Channel Status/present bit error, [3]:no input clock, [2]: HDMI AVMUTE, [1]:clcok big change, [0]: CTS/N over range.
...
void drv_mstar_HDMIRst( BYTE rst )
{
    ...
    {
        msWriteByteMask ( REG_1417, BIT7, BIT7 );
        msWriteByteMask ( REG_1417, 0x00, BIT7 );
                
        msWriteByteMask ( REG_1410, 0x02, 0x02 );
...
void drv_CheckAudioErrorStatus( void )
{
...
#if ENABLE_CTSN_FILTER
    uAudioMuteEn |= (msReadByte(REG_1429) & BIT0) | (msReadByte(REG_14D2) & BIT0);
#endif
#if ENABLE_AUDIO_AUTO_MUTE
    uAudioMuteEn |= msRead2Byte(REG_1428) & 0xFF00; // auto mute events
#endif
#if ENABLE_AUDIO_AUTO_FADING
    uAudioMuteEn |= msRead2Byte(REG_14D1) & 0xFF00; // auto fading events
#endif
    ...
    if( uAudioMuteEn )
    {
        ...
        {
            if( msReadByte( REG_165C ) & _BIT2 ) // error status bit
            {
                gScInfo.ucAudioErrorEvents.bVideoClockBigChanged = 1;
                if( msReadByte( REG_165C ) & _BIT3 ) // video clock is stable
                {
                    msWriteBit( REG_165C, TRUE, _BIT2 ); // write 1 to clear
                }
            }
            else
        ...
        {
            if( msReadByte( REG_14DF ) & _BIT7 ) // error status bit
        ...
        {
            if( msReadByte( REG_14EC ) & _BIT4 ) // error status bit
            {
                gScInfo.ucAudioErrorEvents.bFlatBitReceived = 1;
                //msWriteBit( REG_14EC, TRUE, _BIT4 ); // write 1 to clear
                msWriteByte( REG_14EC, _BIT4 ); // write 1 to clear
            }
            else
                gScInfo.ucAudioErrorEvents.bFlatBitReceived = 0;
        }

        if( uAudioMuteEn & _BIT13 ) // audio present bit error
        {
            if( msReadByte( REG_14DB ) & _BIT7 ) // error status bit
            {
                gScInfo.ucAudioErrorEvents.b2CHASPPresentBitError = 1;
                msWriteBit( REG_17AB, TRUE, _BIT3 ); // clear
                msWriteBit( REG_17AB, FALSE, _BIT3 );
            }
            else
        ...
        if( uAudioMuteEn & _BIT15 ) // audio sample error/audio sample priority error
        {
            if( gScInfo.InputPacketError & _BIT5 ) // error status bit
            {
                ...
                msWriteBit(REG_1422, TRUE, _BIT1); // clear bit
                msWriteBit(REG_1422, FALSE, _BIT1);
                msWriteBit(REG_14EE, TRUE, _BIT5);
            }
            else
